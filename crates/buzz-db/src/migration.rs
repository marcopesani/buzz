...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        // Lightning Address (LUD-16) on users — additive, never folded into 0001.
        assert_eq!(migrations[24].version, 25);
        assert_eq!(&*migrations[24].description, "users lud16");
        assert!(migrations[24]
            .sql
            .as_str()
            .contains("ALTER TABLE users ADD COLUMN lud16"));
        assert!(!migrations[0].sql.as_str().contains("lud16"));
    }
